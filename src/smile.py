...
class Smile:
    ...
    def __init__(self, instrument, maturity, data):
        ...
        if data["calls"] is None:
            ...
            self.calls = callbs(self.fwd, 0., t, self.strikes, self.mids)
        else:
            self.calls = data["calls"]
            
        if data["puts"] is None:
            t = torch.tensor(maturity.t, device=self.fwd.device)
            self.puts = putbs(self.fwd, 0., t, self.strikes, self.mids)
        else:
            self.puts = data["puts"]
    ...
    def from_json(js):
        ...
        strikes = torch.tensor(js["strikes"], )
