...
def wrap_loss_func(date, params, smiles, maturities, gen, dt, fSPX):
    ...
    bm = torchsde.BrownianInterval(t0=T0.t, t1=T_last.t + 2 * dt, 
                               dt=dt, size=[batch_size, 2],
                               device="cuda:0", entropy=42)

    spot = fSPX(torch.tensor(0.))
    ...
    def loss_func(epoch, trajectories=False):
        ...
        model_smiles = {
            maturity.string : {} for maturity in spx_vix_maturities
        }
        ...
        for i, maturity in enumerate(all_maturities):
            if maturity in spx_maturities:
                ...
                loss_spx = compute_loss_smile_spx(smile_spx, spx_smiles[maturity])
            ...
            if maturity in vix_maturities:
                ...
                loss_fVIX, loss_CVIX = compute_loss_smile_vix(smile_vix, vix_smiles[maturity])
                loss_dict["fVIX"] += loss_fVIX
                loss_dict["CVIX"] += loss_CVIX

        loss = (w_fVIX * loss_dict["fVIX"] / n_vix 
...
def prepare_data(data, date, maturities):
    ...
    spx_maturities = sorted([Datetime(maturity, T0=date) for maturity in maturities["spx"]])
    vix_maturities = sorted([Datetime(maturity, T0=date) for maturity in maturities["vix"]])


    spx_smiles = OrderedDict({
        maturity : tensorize_smile(data["spx_smiles"], maturity, "SPX market", 40, 0.90, 1.05)
        for maturity in spx_maturities
    })

    vix_smiles = OrderedDict({
        maturity : tensorize_smile(data["vix_smiles"], maturity, "VIX market", 40, 0.85, 1.85)
    ...
    all_maturities = sorted(set(spx_maturities + vix_maturities + vix_maturities_plus30d))
