...
def compute_vol(fwd, maturity, strikes, prices, cp_flag):
    asks_vol = np.array([
        implied_vol_np(fwd, 0., maturity.t, k, c, cp_flag, nan=True) for k, c in zip(strikes, prices["asks"])
    ])
    bids_vol = np.array([
        implied_vol_np(fwd, 0., maturity.t, k, c, cp_flag, nan=True) for k, c in zip(strikes, prices["bids"])
    ])
...
def extract_spx_smile(spx, maturity):
    ...
    strikes_c, vol_c_asks, vol_c_bids = compute_vol(fwd, maturity, bid_ask["strikes"], bid_ask["calls"], "C")
    strikes_p, vol_p_asks, vol_p_bids = compute_vol(fwd, maturity, bid_ask["strikes"], bid_ask["puts"], "P")
    ...
    bid_ask["vol"] = {
        "c" : {
            "strikes" : strikes_c, "asks" : vol_c_asks, "bids" : vol_c_bids
        },
        "p" : {
            "strikes" : strikes_p, "asks" : vol_p_asks, "bids" : vol_p_bids
        },
    }
    
    
    smile_strikes = np.concatenate([strikes_p[strikes_p < fwd], strikes_c[strikes_c >= fwd]])
    smile_asks = np.concatenate([vol_p_asks[strikes_p < fwd], vol_c_asks[strikes_c >= fwd]])
    smile_bids = np.concatenate([vol_p_bids[strikes_p < fwd], vol_c_bids[strikes_c >= fwd]])
...
def extract_yield_curve(yield_curve):
    return {
    "days" : yield_curve.days.values,
    "rate" : yield_curve.rate.values
    }
...
def plot(smiles, maturities):
    ...
    for i, maturity in enumerate(maturities):
        ...
        plt.scatter(strikes, bid_ask["fwd"]["asks"], color="blue", label="forward ask", s=5)
        plt.axhline(fwd, color="black", label="chosen forward")
        plt.scatter(strikes, bid_ask["fwd"]["bids"], color="green", label="forward bid", s=5)
        plt.xlabel("strike")
        plt.ylabel("C - P + K")
        plt.legend()
        plt.title(maturity)
        ...
        plt.scatter(bid_ask["vol"]["c"]["strikes"], bid_ask["vol"]["c"]["asks"], color="blue", s=1, label="calls implied vol. ask")
        plt.scatter(bid_ask["vol"]["c"]["strikes"], bid_ask["vol"]["c"]["bids"], color="blue", s=1, label="calls implied vol. bid")
        plt.fill_between(bid_ask["vol"]["c"]["strikes"], bid_ask["vol"]["c"]["bids"], bid_ask["vol"]["c"]["asks"], color="blue", alpha=0.3)

        plt.scatter(bid_ask["vol"]["p"]["strikes"], bid_ask["vol"]["p"]["asks"], color="green", s=1, label="puts implied vol. ask")
        plt.scatter(bid_ask["vol"]["p"]["strikes"], bid_ask["vol"]["p"]["bids"], color="green", s=1, label="puts implied vol. ask")
        plt.fill_between(bid_ask["vol"]["p"]["strikes"], bid_ask["vol"]["p"]["bids"], bid_ask["vol"]["p"]["asks"], color="green", alpha=0.3)
        ...
        plt.fill_between(smile["strikes"], smile["bids"], smile["asks"], color="red", alpha=0.3, label="bid-ask")
        plt.scatter(smile["strikes"], smile["bids"], color="red", s=5)
        plt.plot(smile["strikes"], smile["mids"], color="red", label="mid", linestyle="--")
        plt.axvline(fwd, color="black", label="forward")
